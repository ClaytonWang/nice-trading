  ...
  <page-layout>
    <detail-list slot="headerContent" size="small" :col="2">
      ...
      <detail-list-item
        ...
        :span="2"
      >
        <template v-html="tradingPlan.comments.comment" />
      </detail-list-item>
    </detail-list>
...
import { create, update } from '@/services/trading-detail'
import { fetch } from '@/services/trading-plan'
const DetailListItem = DetailList.Item
export default {
  name: 'TradeDetail',
  components: { DetailList, DetailListItem, PageLayout, DetailForm },
  data() {
    return {
      tradingPlan: {
        strategy: { title: '' }
        ...
        if (!err) {
          ...
          if (this.isEdit) {
            ...
            update(detail).then(res => {
              ...
              this.tradingPlan.trading_details.splice(index, 1, res.data)
              this.showDlg = false
            })
          } else {
            ...
            create(detail).then(res => {
              ...
              this.tradingPlan.trading_details.push(res.data)
              this.showDlg = false
            })
          }
        }
